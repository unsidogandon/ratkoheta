...
def encrypt(text):
    ...
    second_xor_result = "".join(
        chr(ord(c) ^ ord(k)) for c, k in zip(xor_with_noise, second_key)
    )
...
def decrypt(shuffled, key, second_key, indices, noise_length):
    ...
    xor_with_noise = "".join(
        chr(ord(c) ^ ord(k)) for c, k in zip(second_xor_result, second_key)
    )
...
class CipherMod(loader.Module):
    ...
    async def deccmd(self, message):
        ...
        if len(args) < 5 and not reply:
            await utils.answer(
                message,
                "Please reply to a message or provide <b>encrypted text</b>, <b>key1</b>, <b>key2</b>, <b>indices</b>, <b>noise length</b>.",
            )
            return
        if reply:
            enc_data = ""
        ...
        indices = (
            ...
            if not reply
            else list(map(int, enc_data.split()[3].split(",")))
        )
