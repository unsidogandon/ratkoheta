...
# meta tags: music, soundcloud, download, playlist, search, heroku, музыка, саундклауд, скачивание, плейлист, поиск, хероку
# authors: @goymodules
...
def _fetch_sync(url: str) -> bytes:
    headers = {"User-Agent": "SoundCloudMusic-Module/4.0 (Core by @samsepi0l_ovf; AGPLv3)"}
...
class SoundCloudMusic(loader.Module):
    """Тулза для дампа музыки из SoundCloud с карточками и плейлистами. Парить себе мозг с локальным окружением не нужно."""

    strings = {
        "name": "SoundCloudMusic",
        "no_query": "<tg-emoji emoji-id=5256079005731271025>📟</tg-emoji> <b>Аргументы где?</b> Укажи таргет SoundCloud или сделай реплай на аудио.",
        "no_tracks": "<tg-emoji emoji-id=5256025060942031560>🐢</tg-emoji> <b>Ничего не найдено.</b> SoundCloud молчит.",
        "searching": "<tg-emoji emoji-id=5255971360965930740>🕔</tg-emoji> <b>Паршу SoundCloud...</b>",
        ...
        "pl_list": "<tg-emoji emoji-id=5253526631221307799>📂</tg-emoji> <b>Локальные БД (Плейлисты SoundCloud):</b>\nВыбирай с чем работать:",
    ...
    def __init__(self):
        ...
        self.storage_dir = os.path.join(os.getcwd(), "soundcloud_storage")
    ...
    def _normalize_track(self, raw: dict, source: str = None) -> dict:
        ...
        uploader = (
            ...
            or raw.get("publisher_metadata", {}).get("artist")
            or "Unknown"
        )
        normalized = {
            "id": track_id,
            ...
            "source": source or raw.get("source") or "sc",
            "_ovf_sign": raw.get("_ovf_sign") or "https://github.com/sepiol026-wq/",
        }
        for key in ("channel", "artist", "thumbnail", "thumbnails", "webpage_url", "artwork_url", "user", "publisher_metadata"):
            if raw.get(key):
    ...
    def _extract_cover_url(self, track_info: dict, meta: dict = None) -> str:
        ...
        for source in (track_info or {}, meta or {}):
            for key in ("artwork_url", "thumbnail", "thumbnail_url", "avatar_url"):
                thumb = source.get(key)
                if thumb:
    ...
    async def _download_soundcloud_track(self, track_url: str, out_path: str) -> dict:
        async with requests.AsyncSession(impersonate="chrome124") as ses:
            h_resp = await ses.get(track_url)
            if h_resp.status_code != 200:
                raise ValueError("SoundCloud page not reachable")
            ...
            if not h_m:
                raise ValueError("No hydration payload")

            payload = json.loads(h_m.group(1))
            t_d = next((i.get("data") for i in payload if i.get("hydratable") == "sound"), None)
            ...
            if not tr:
                raise ValueError("No transcodings")
            ...
            if s_resp.status_code != 200:
                raise ValueError("Stream metadata failed")

            s_json = s_resp.json()
            media_url = s_json.get("url")
            if not media_url:
                raise ValueError("No stream url")

            protocol = s_info.get("format", {}).get("protocol")
            if protocol == "progressive":
                m_resp = await ses.get(media_url)
                if m_resp.status_code != 200:
                    raise ValueError("Audio download failed")
                with open(out_path, "wb") as file_obj:
                    file_obj.write(m_resp.content)
            else:
                m3_resp = await ses.get(media_url)
                if m3_resp.status_code != 200:
                    raise ValueError("Playlist download failed")
                chunks = [l.strip() for l in m3_resp.text.splitlines() if l and not l.startswith("#")]
                if not chunks:
                ...
                async def _fetch_segment(url: str, index: int):
                    if not url.startswith("http"):
                        url = base_url + url
                    async with sem:
                        resp = await ses.get(url)
                        if resp.status_code != 200:
                            raise ValueError(f"Segment {index} failed")
                        return index, resp.content

                segments = await asyncio.gather(*[_fetch_segment(u, i) for i, u in enumerate(chunks)])
            ...
            return {
                "id": str(t_d.get("id") or ""),
                "title": t_d.get("title") or "Unknown",
                "uploader": (
                    t_d.get("user", {}).get("username")
                    or t_d.get("publisher_metadata", {}).get("artist")
                    or t_d.get("user", {}).get("full_name")
                    or "Unknown"
                ),
                "duration": int(t_d.get("duration") or 0) // 1000,
                "thumbnail": art,
                "webpage_url": track_url,
                "source": "sc",
            }

    async def _ensure_soundcloud_track_cached(self, track_info: dict):
        track_info = self._normalize_track(track_info, source="sc")
        ...
        async with lock:
            ...
            dl_dir = tempfile.mkdtemp(prefix="scmusic_cache_")
            try:
                track_url = (
                    track_info.get("webpage_url")
                    or track_info.get("url")
                    or track_info.get("original_url")
                    or track_info.get("permalink_url")
                    or ""
                )
                if not track_url:
                    uploader = track_info.get("uploader") or "unknown"
                    track_url = f"https://soundcloud.com/{uploader}/{track_id}"

                mp3_path = os.path.join(dl_dir, f"{track_id}.mp3")

                try:
                    meta = await self._download_soundcloud_track(track_url, mp3_path)
                except Exception as sc_exc:
                    logger.warning("SoundCloud direct download failed for %s: %s", track_id, sc_exc)
                    ...
                    if returncode != 0 or not os.path.exists(mp3_path):
                        raise RuntimeError(self._parse_sc_error(stderr))
    ...
    def _parse_sc_error(self, stderr_bytes):
        text = stderr_bytes.decode('utf-8', errors='ignore').lower()
        if "timeout" in text or "timeouterror" in text:
            return "Таймаут соединения. SoundCloud не ответил вовремя."
        if "403" in text:
            return "SoundCloud выдал 403 (Forbidden). IP или сессия не проходят."
        if any(e in text for e in ["name or service not known", "unreachable", "connection reset"]):
            return "Соединение разорвано (возможно блок DPI/DNS). Проверь хост."
        if "sign in" in text or "captcha" in text:
            return "Требуется авторизация (капча/логин)."
        ...
        if stdout:
            for line in stdout.decode('utf-8', errors='ignore').split('\n'):
                if not line.strip():
                    continue
                try:
                    track = self._normalize_track(json.loads(line), source="sc")
                    if track:
    ...
    async def _dl_and_send(self, call_or_msg, track_info, target_chat_id):
        track_info = self._normalize_track(
            track_info, source=track_info.get("source", "sc")
        )
        track_id = str(track_info.get("id"))
        source = track_info.get("source", "sc")
        ...
        sc_url = (
            ...
            or track_info.get("url")
            or track_info.get("permalink_url")
            or f"https://soundcloud.com/{track_info.get('uploader', 'unknown')}/{track_id}"
        )
        try:
            paths, meta = await self._ensure_soundcloud_track_cached(track_info)
        ...
        try:
            ...
            caption = "<tg-emoji emoji-id=5256079005731271025>📟</tg-emoji> <b>Таргет:</b> {track}\n<tg-emoji emoji-id=5255835635704408236>👤</tg-emoji> <b>Автор:</b> {artists}\n<tg-emoji emoji-id=5253490441826870592>🔗</tg-emoji> <b>Лонг-линк:</b> <a href='{sc_url}'>Сурс</a>".format(**sdata)
        ...
        with contextlib.suppress(Exception):
            await call_or_msg.delete()
    @loader.command(ru_doc="Экспорт всех БД в файл", alias="scexport")
    async def scexportcmd(self, message: Message):
        ...
        out.name = "soundcloud_music_export.json"
    ...
    @loader.command(ru_doc="Импорт БД из файла (реплай)", alias="scimport")
    async def scimportcmd(self, message: Message):
    ...
    async def scbatchcmd(self, message: Message):
        ...
        if not args or " " not in args:
            return await utils.answer(message, "<tg-emoji emoji-id=5256079005731271025>📟</tg-emoji> <b>Формат:</b> <code>.scbatch [БД] https://soundcloud.com/artist/sets/album</code>")

        pl_name, url = args.split(" ", 1)
        msg = await utils.answer(message, "<tg-emoji emoji-id=5255971360965930740>🕔</tg-emoji> <b>Паршу плейлист SoundCloud пачкой...</b>")
        ...
        for line in stdout.decode('utf-8', errors='ignore').split('\n'):
            ...
            try:
                ...
                track = self._normalize_track(data, source="sc")
                if not track:
                    continue
    ...
    @loader.command(ru_doc="<запрос> - Дамп трека из SoundCloud", alias="sc")
    async def sccmd(self, message: Message):
    ...
    @loader.command(ru_doc="Менеджер локальных БД (плейлистов)", alias="scpl")
    async def scplcmd(self, message: Message):
        """Playlists manager"""
        target_chat_id = utils.get_chat_id(message)
        playlists = self._get_playlists()
        self._set_playlists(playlists)
    ...
    @loader.command(ru_doc="<база> <запрос/реплай> - Инжект трека/войса в базу", alias="scadd")
    async def scaddcmd(self, message: Message):
        """<playlist> <query/reply> - Add to playlist"""
        args = utils.get_args_raw(message)
        reply = await message.get_reply_message()

        if not args:
            return await utils.answer(message, "<tg-emoji emoji-id=5256079005731271025>📟</tg-emoji> <b>Формат:</b> <code>.scadd [БД] [Таргет/Реплай на аудио/войс]</code>")
    ...
    @loader.command(ru_doc="<база> <имя_трека> - Удалить трек из базы", alias="scrm")
    async def scrmcmd(self, message: Message):
        """<playlist> <query> - Remove from playlist"""
        args = utils.get_args_raw(message)
        if not args or " " not in args:
            return await utils.answer(message, "<tg-emoji emoji-id=5256079005731271025>📟</tg-emoji> <b>Формат:</b> <code>.scrm [БД] [Имя трека]</code>")
